...
function apply(ctx) {
	...
	/** 从 scene.pkg（Wallpaper Engine 私有 PKGV 容器）中扫描最大的 JPEG/PNG 纹理 */
		...
		while (pos < buf.length - 4) {
			if (buf[pos] === 255 && buf[pos + 1] === 216 && buf[pos + 2] === 255) {
				...
				for (let guard = 0; scan < buf.length - 9 && guard < 64; guard++) {
					if (buf[scan] !== 255) { scan++; continue; }
					const marker = buf[scan + 1];
					if (marker === 216 || marker >= 208 && marker <= 215) { scan += 2; continue; }
	 ...
	 *  与 DSH 主源隔离，给 Spine/WebGL 壁纸一个自己的同源环境。 */
	...
	try {
		...
		sourceServer.listen(0, "127.0.0.1", () => {
			const addr = sourceServer === null ? null : sourceServer.address();
			if (addr !== null && typeof addr === "object") webPort = addr.port;
		});
		disposers.push(() => {
			if (sourceServer !== null) {
				try { sourceServer.close(); } catch {}
			}
		});
	} catch { webPort = 0; }
