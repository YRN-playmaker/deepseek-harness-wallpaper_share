		...
		handler(_req, res) {
			(async () => {
				...
				try {
					...
					let spawnError = "";
					const child = spawn(state.weDir + "/wallpaper64.exe", [
						"-control",
						"openWallpaper",
						"-workshop",
						pick
					...
					if (spawnError !== "") {
						sendJson(res, {
							ok: false,
							error: "无法启动 wallpaper64.exe：" + spawnError
						});
						return;
					}
					...
					while (Date.now() < deadline) {
						...
						try {
							...
							for (const key of Object.keys(entries)) {
								const entry = entries[key];
								if (entry !== void 0 && entry.file.includes(needle)) {
									poll(state.weDir);
									sendJson(res, {
										ok: true,
										workshopId: pick
					...
					sendJson(res, {
						ok: false,
						error: "Wallpaper Engine 未在 " + Math.round(CONFIG.randomVerifyMs / 1e3) + " 秒内应用该壁纸（可能被忽略或壁纸损坏）"
					});
				} catch (e) {
					sendJson(res, {
						ok: false,
