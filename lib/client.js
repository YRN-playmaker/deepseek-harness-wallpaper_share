		...
		function WallpaperSharePanel() {
			...
			const onRenderMode = () => {
				...
				if (next === "source") {
					...
					else if (kind === "image") flash("增强模式：显示壁纸源图");
					else if (kind === "scene") flash(store.info !== null && store.info.source.scene ? "增强模式：显示场景高清纹理（提取自 scene.pkg）" : "该场景包无法提取纹理，回退预览图");
					else flash("当前壁纸（" + (kind === "" ? "无" : kind) + "）无法增强，回退预览图");
		...
		function apply(ctx) {
			...
			function applyBackground() {
				...
				if (enabled && info !== null) {
					...
					else if (sourceKind === "scene") imgUrl = "url(\"/we-sync/scene?monitor=" + encodeURIComponent(monitorKey) + "&v=" + info.version + "\")";
