		...
		*/
		function WallpaperSharePanel() {
			...
			const onRandom = async () => {
				...
				setBusy(true);
				try {
					const result = await (await fetch("/we-sync/random", { cache: "no-store" })).json();
			...
			const subtitle = wallpaper === null ? "在 Wallpaper Engine 中应用壁纸后，此处会同步显示" : wallpaper.type + (info !== null && info.kind === "image" ? " · 已同步静态预览" : " · 无静态预览图");
		...
		function apply(ctx) {
			...
			function applyBackground() {
				...
				const img = store.settings.enabled && info !== null && info.kind === "image" ? "url(\"/we-sync/preview?v=" + info.version + "\")" : "none";
